...
public class PodcastAndEpisodeAttributesServiceImpl implements PodcastAndEpisodeAttributesService {
	...
	private static final String NO_IMAGE_LOCAL_URL = "http://www.podcastpedia.org/static/images/podcast-tm.jpg";
	...
	public void setPodcastFeedAttributes(Podcast podcast) throws IllegalArgumentException,
		...
		SyndFeed syndFeed = syndFeedService.getSyndFeedForUrl(podcast.getUrl());
		...
		if (syndFeed != null) {
			...
			if (null != podcastImage) {
				if (podcastImage.getUrl() != null) {
					...
					podcast.setUrlOfImageToDisplay(NO_IMAGE_LOCAL_URL);
				}
			} else {
				podcast.setUrlOfImageToDisplay(NO_IMAGE_LOCAL_URL);
			}
			...
			for (SyndEntryImpl entry : (List<SyndEntryImpl>) syndFeed
					.getEntries()) {
				// get the list of enclosures
				List<SyndEnclosureImpl> enclosures = (List<SyndEnclosureImpl>) entry
						.getEnclosures();
		...
		if (syndFeed != null) {
			...
			if (null != podcastImage) {
				if (podcastImage.getUrl() != null) {
					...
					newImageUrl = NO_IMAGE_LOCAL_URL;
				}
			} else {
				newImageUrl = NO_IMAGE_LOCAL_URL;
			}
